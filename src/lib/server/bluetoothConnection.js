...
export function isBluetoothDeviceConnected({ force = false, now = Date.now(), run } = {}) {
	...
	try {
		const exec =
			run ||
			((args, env) =>
				execFileSync('bluetoothctl', args, {
					encoding: 'utf8',
					timeout: 2000,
					env,
					stdio: ['ignore', 'pipe', 'pipe']
				}));
		const out = exec(['--timeout', '3', 'devices', 'Connected'], sessionEnv());
